...
import cn.ksmcbrigade.mr.transformers.HotMixinTransformer;
...
public class MixinUtils {
    ...
    public static Config toConfig(String configFile){return Config.create(configFile);}

    public static void getMixins(String configFile) {
        ...
        for (String s : MixinConfigUtils.getGlobalMixinList(iMixinConfig)) {
            try {
                for (Class<?> targetClass : getTargetClasses(Class.forName(s))) {
                    Objects.requireNonNull(MixinAgentUtils.getInst()).retransformClasses(targetClass);
                }
            } catch (Throwable e) {
                e.printStackTrace();
            }
